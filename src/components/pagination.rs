...
fn page_button(
    ...
    if active {
        // Opaque `widget` surface — `background` is the (potentially
        // translucent) app canvas token, and `card` is reserved for static
        // containers, not this interactive page button.
        ui.painter().rect(
            rect,
            tokens.radius_md(),
            tokens.widget,
